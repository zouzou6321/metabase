...
const QuestionSidebar = ({ card, originalCard, tableMetadata, onShowMetric, onShowQuestionDetails }: Props) =>
    <Sidebar>
        { card.id != null ?
            <SidebarSection>
        ...
        : card.id == null && originalCard ?
            <SidebarSection>
                <span>started from <span className="link">{originalCard.name}</span></span>
            </SidebarSection>
        : null }
        { getMetrics(card, tableMetadata).map((metric, metricIndex) =>
            <SidebarSection className="h3 text-bold text-grey-4 cursor-pointer" onClick={() => onShowMetric(metricIndex)}>
                {metric.name}
            </SidebarSection>
        )}
